...
break;
if (digitIndex < wcslen(lowDigits) && (index+1) < str->length()) {
*arrStr[arrCount-1] += L"#";
while (true) {
wcStr[0] = L'a'+digitIndex;
wcStr[1] = L'\0';
...
break;
if (digitIndex < wcslen(lowDigits) && (index2+1) < str->length())
createFrac = true;
}  // if (digitIndex < wcslen(upDigits)) {
if (createFrac) {
if (index > startBrl) {
...
wcStr[0] = 0xf023;
wcStr[1] = wcNULL;
arrStr[startFrac[--startFracCount]] = new CString(wcStr);
wcStr[0] = 0xf024;
wcStr[1] = wcNULL;
arrStr[midFrac[--midFracCount]] = new CString(wcStr);
wcStr[0] = 0xf021;
wcStr[1] = wcNULL;
arrStr[arrCount] = new CString(wcStr);
arrCount++;
...
if (index > startBrl && startBrl < str->length()) {
arrStr[arrCount] = new CString(L" ");
*arrStr[arrCount] += str->substring(startBrl, index);
arrCount++;
}
...
wprintf(L"$%d: \"%s\"\n", i, (wchar_t*) strTranslation->c_str());
translated_buffer = NULL;
back_translate_with_mathexpr_table((widechar*) strTranslation->c_str(), strTranslation->length(), &translated_buffer, &translatedLength);
delete strTranslation;
wprintf(L"^%d: \"%s\"\n", i, (wchar_t*) translated_buffer);

	CDOMElement* node = parseMath((wchar_t*) translated_buffer);
