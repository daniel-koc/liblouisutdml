...
class CString {
 public:
  ...
  int length() const { return (int)m_nStrLen; }

  bool isEmpty() const { return (m_nStrLen == 0); }

  wchar_t* c_str() { return m_pszStr; }

  operator const wchar_t*() const { return m_pszStr; }

  operator wchar_t*() const { return m_pszStr; }
  ...
  bool equals(wchar_t* pszStr) const { return (wcscmp(m_pszStr, pszStr) == 0); }
  ...
  wchar_t& operator[](int nIndex) {
    return m_pszStr[((nIndex < 0) ? (0) : (((size_t)nIndex >= m_nStrLen)
                                               ? (m_nStrLen - 1)
                                               : (nIndex)))];
  }
