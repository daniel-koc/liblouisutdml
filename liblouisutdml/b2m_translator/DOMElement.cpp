...
{
CDOMElement* pNewElement = m_pOwnerDocument->CreateElement(new CString(*m_pStrTagName));
...
pTmpAttr->GetPreviousNodeSibling()->SetNextNodeSibling(pTmpAttr->GetNextNodeSibling());
else
m_pFirstAttr = (CDOMAttr*) pTmpAttr->GetNextNodeSibling();
if (pTmpAttr->GetNextNodeSibling() != NULL)
pTmpAttr->GetNextNodeSibling()->SetPreviousNodeSibling(pTmpAttr->GetPreviousNodeSibling());
else
